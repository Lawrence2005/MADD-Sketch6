...
@torch.no_grad()
def generate_samples(checkpoint=CHECKPOINT_PATH, sample_dir=SAMPLE_DIR, latent_dim=LATENT_DIM, num_samples=16):
...
def train(image_dir=RAW_IMG_DIR, image_size=IMAGE_SIZE, batch_size=BATCH_SIZE, latent_dim=LATENT_DIM, learning_rate=LEARNING_RATE, 
            num_epochs=EPOCHS, beta=BETA, outputs_dir=OUTPUTS_DIR, ckpt_path=CHECKPOINT_PATH):
    ...
    Path(SAMPLE_DIR).mkdir(parents=True, exist_ok=True)
    for epoch in range(15, 15 + num_epochs):
        model.train()
